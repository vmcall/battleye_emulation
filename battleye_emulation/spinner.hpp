...
public:
	...
	inline void start()
	{
		if (this->m_active)
		{
			loggr::log_error("Cannot initialise spinner object more than once!");
			return;
		}
	...
	inline void update()
	{
		if (!this->m_active)
		{
			loggr::log_error("Cannot initialise spinner object without initialization!");
			return;
		}
		...
		loggr::clear_line();
		loggr::log_raw("[{}]", static_cast<char>(spinner::CHARACTER_SEQUENCE[this->m_seq_index]));
	...
	inline void stop()
	{
		if (!this->m_active)
		{
			loggr::log_error("Spinner is not active!");
			return;
		}
